...
def check_box(event=None):
    print 'check_box'
    file1=open('finacial.log','w')
    count=1
    for i in xrange(len(li)):
        #print v[i].get()
        if v[i].get() ==1:
            print li[i]
            file1.write(str(count)+':'+li[i][:len(li[i].strip())]+'\n')
            count=count+1
            ...
            sql = '''insert into  daily_reports(s_date,title,link,title_hash,report_type)values ('%s','%s','%s','%s','%s')
                                    ''' % (datetime.date.today(), title, link, title_hash, 'finacial_bankjrj')
...
if __name__ == '__main__':
    ...
    sql = "select title_hash from daily_reports where s_date>=%s and report_type='finacial' " % (GetDate(7))
    cursor.execute(sql)
    ...
    sql = "select title,link,title_hash,source from dailyfinacial where s_date=%s" % (GetDate(0))
    cursor.execute(sql)
