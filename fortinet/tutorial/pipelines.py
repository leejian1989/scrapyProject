...
class TutorialPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        if len(item['title'])>0 and len(item['link'])>0:
            self.file.write('fortinet\n')
            #self.file.write(str(len(item['title']))+' '+str(len(item['info'])))
            for i in xrange(len(item['title'])):
                '''Mar 7, 2017 '''
                dat=item['dat'][i].replace(',','')
                dat_list=dat.split()
                if len(dat_list)!=3:
                    print 'date is not right!!!!'
                #print 'date--------',dat_list[2],dat_list[0]

                dat_1=datetime.date(int(dat_list[2]),month[dat_list[0]],int(dat_list[1]))
                dat_t=datetime.date.today()
                if (dat_t-dat_1).days<=2:
                    print 'date--------',dat_1,dat_t,(dat_t-dat_1).days
                    print 'title-------',item['title'][i]
                    self.file.write(str(i+1)+': ')
                    self.file.write(str(item['title'][i].strip().encode("utf-8"))+str('http://blog.fortinet.com')+str(item['link'][i])+'\n')

                    dict_item = {
                        's_date': datetime.date.today(),
                        'title': str(item['title'][i].strip().encode("utf-8")),
                        'source': 'fortinet',
                        'link': str('http://blog.fortinet.com')+str(item['link'][i].strip()),
